/* ===============================
   ✅ EVENT PAGE SCRIPT (FIXED + SAFE)
   - Post Date (auto today)
   - Theme toggle (localStorage)
   - Image modal (open/close + ESC)
...
(() => {
  ...
  (function () {
    ...
    // If modal not on this page, don't crash
    if (!imgModal || !imgModalPic) return;

    // expose to html: onclick="openImg(this.src)"
    ...
    imgModal.addEventListener("click", (e) => {
      if (e.target === imgModal) window.closeImg();
    });
  ...
  /* ---------------------------
     🎵 BFN MUSIC SYSTEM (SAFE)
     IDs expected:
     #bgm #songName #songStatus #trackPill #modePill #miniStatus
     #prevBtn #playBtn #nextBtn #shuffleBtn #volRange
     #musicWidget #minBtn #expandBtn
  --------------------------- */
  (function () {
    ...
    // If missing on event page, skip safely
    const hasMusicUI =
      bgm &&
      songName &&
      songStatus &&
    ...
    function nextTrack() {
      const nextIndex = shuffleOn ? pickRandomIndex(currentIndex) : (currentIndex + 1) % playlist.length;
      loadTrack(nextIndex);
      safePlay();
    }

    function prevTrack() {
      const prevIndex = shuffleOn ? pickRandomIndex(currentIndex) : (currentIndex - 1 + playlist.length) % playlist.length;
      loadTrack(prevIndex);
      safePlay();
    }
    ...
    // Autoplay on first real interaction (event page friendly)
    function shouldTriggerBGM(target) {
      if (!target || !target.closest) return false;
      return target.closest("a, button, .btn, .poster, .logo, .nav, .sidebar, .music-widget, main, .content");
    }
    ...
    async function firstUserStart(e) {
      ...
      document.removeEventListener("touchstart", firstUserStart);
    }

    document.addEventListener("pointerdown", firstUserStart, { passive: true });
    document.addEventListener("click", firstUserStart);
    document.addEventListener("touchstart", firstUserStart, { passive: true });
